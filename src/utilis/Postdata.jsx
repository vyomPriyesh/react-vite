...
const Postdata = ({ title, moreData, profile, heroData }) => {
  ...
  useEffect(() => {
    ...
    const metaTags = [
      ...
      { property: 'og:image', content: `https://img.youtube.com/vi/${profile?.video_img}/0.jpg` },
      { property: 'og:url', content: `${protocol}//${host}${port ? `:${port}` : ''}/?nid=${profile?.share}` },
      { property: 'og:site_name', content: 'Info Gujrat' },
    ];

    metaTags.forEach((tag) => {
      const metaElement = document.querySelector(`meta[property="${tag.property}"]`);
      if (metaElement) {
  ...
  }, [title, moreData, profile, protocol, host, port]);
